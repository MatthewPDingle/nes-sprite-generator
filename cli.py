...
def process_single_version(version, args, generator, total_versions):
    ...
    Args:
        ...
        generator: PixelArtGenerator instance
    ...
    try:
        ...
        # Log the original dimensions before any processing
        orig_height = len(pixel_data["pixel_grid"])
        orig_width = len(pixel_data["pixel_grid"][0]) if orig_height > 0 else 0
        if orig_height != args.height or orig_width != args.width:
            logger.warning(f"Model generated incorrect dimensions: {orig_width}x{orig_height}, requested: {args.width}x{args.height}")
        
        # Process and save the image - always use post-processing to ensure correct dimensions
        result = generator.process_image(
            pixel_data=pixel_data,
            output_file=version_output,
            post_process=True,  # Always post-process to ensure correct dimensions
...
def handle_single_command(args):
    ...
    generator = PixelArtGenerator(model=args.model)
    ...
    if args.versions > 1:
        ...
        for batch_start in range(1, args.versions + 1, max_workers):
            ...
            with concurrent.futures.ThreadPoolExecutor(max_workers=max_workers) as executor:
                ...
                futures = {
                    executor.submit(process_single_version, version, args, generator, args.versions): version
...
def list_available_models():
    """
    Format and return a string listing all available models with descriptions.
